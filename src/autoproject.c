...
void convert(icStructCFG *pcfg, char *nmFile, char *fileScript)
{
    ...
    {   
        ...
        oBook = open_workbook_xls(FNameValue, pcfg->bCheckINN);
            ...
            if (num >= 0 && result >= 0)
            {
                    ...
                    strequal(strleft(cmdWord, 2), "tf")) 
                {
                    ...
                    setRangeFont(oBook, oSheet, 
                                 ...
                                 pClr->bBold, pClr->bItalic, pClr->bUnderline, pcfg->bCheckINN);
...
static BOOL _drawTabHead(lxw_workbook *oBook, lxw_worksheet *oSheet,
                        ...
                        char *strCell, int step, int maxCell, BOOL bHead, BOOL bCheckINN)
{
    ...
    {
        // выделяем шапку
        setRangeBorder(oBook, oSheet, cell1, cell2, 0, LXW_BORDER_THIN, 0, LXW_BORDER_THIN, bCheckINN);
        setRangeBorderAround(oBook, oSheet, cell1, cell2, LXW_BORDER_MEDIUM, LXW_BORDER_MEDIUM, LXW_BORDER_MEDIUM, LXW_BORDER_MEDIUM, bCheckINN);
    }
    ...
    for (unsigned int n = 2; n < IndexFormat->GetLength(); n++)
    {
        ...
        setRangeBorder(oBook, oSheet, cell1, cell2, LXW_BORDER_THIN, 0, 0, 0, bCheckINN);
    }
                            ...
                            icStructCFG *pcfg, BOOL bClear)
{
    ...
    while (!IsEOF)
    {
        ...
        for (n = 0; (n < boundBuff) && (!bFormatLine) && (((n + step) * 2 + 1) < lenBuffStr); n++)
        {
            ...
            if ((!pcfg->bHead) && (!bTabHead))
            {
                ...
                set_cell_value(oBook, oSheet, row, col, buffStrFreeTag, format, pcfg->bCheckINN);
            }
            else if (pcfg->bHead)
            {
    ...
    for (n=0; (n <= maxCell) && (indexFormat.GetLength() > 0); n++)
    {
        ...
        setRangeBorder(oBook, oSheet, cell1, cell2, 0, LXW_BORDER_THIN, 0, LXW_BORDER_THIN, pcfg->bCheckINN);
        //setRangeBorder(oBook, oSheet, cell2, cell2,  0, 0, LXW_BORDER_THIN, 0, pcfg->bCheckINN);
        ...
        if (oFont)
        {
            ...
            setRangeFont(oBook, oSheet, cellSpcNext, cellSpcNext, 
                         ...
                         oFont->isBold, oFont->isItalic, oFont->isUnderline, pcfg->bCheckINN);
            delete oFont;
            oFont = NULL;
        }
    }
    ...
    _drawTabHead(oBook, oSheet, &indexFormat, strCell, step, maxCell, pcfg->bHead, pcfg->bCheckINN);
...
char *convertNumber(char *str)
{
    ...
    for (n=0; (n < size) && (!bDate); n++)
    {
        ...
        {
            //Это строка
            if (DBG_MODE) logAddLine("<%s> is not number", pstr);    
            bch = strfree(bch);
            pstr = strfree(pstr);
            return str;
        }
...
BOOL convertTXT(icStructCFG *pcfg)
{
    ...
    if (file_exists(pcfg->strExcelName))
        oBook = open_workbook_xls(pcfg->strExcelName, pcfg->bCheckINN);
    else 
    {
